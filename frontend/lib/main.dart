...
class HarApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
    ...
    home: const HarHomeScreen(),
  );
}

class HarHomeScreen extends StatefulWidget {
  const HarHomeScreen({super.key});
  @override
  State<HarHomeScreen> createState() => _HarHomeScreenState();
}

class _HarHomeScreenState extends State<HarHomeScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Stack(
      children: [
        SafeArea(child: Padding(padding: const EdgeInsets.all(32), child: Column(children: [
          _buildTopScope(), const Spacer(), _buildMainStage(), const Spacer(), _buildLogs(), const SizedBox(height: 30), _buildAction(),
        ]))),
  ...
  Widget _buildTopScope() => Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
    Text('HAR CLIENT v2', style: GoogleFonts.outfit(fontWeight: FontWeight.w900, color: const Color(0xFF6366F1))),
  ...
  Widget _buildMainStage() => Column(children: [
    Text(_activity, style: GoogleFonts.outfit(fontSize: 48, fontWeight: FontWeight.w900, color: const Color(0xFF0F172A), letterSpacing: -1)),
    Text('${_conf.toStringAsFixed(1)}% CONFIDENCE', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFF6366F1))),
  ]);

  Widget _buildLogs() => Container(height: 100, width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)), 
